...
class DropZone extends React.Component {
  ...
  render() {
    const { isDisabled, isDragOver, onSelect, onDragOver, onDragLeave, onDrop, dragDropHint, pasteHint, ariaLabel } = this.props;
...
class ImageUpload extends React.Component {
  ...
  handleUploadModalHidden = () => {
    document.removeEventListener("paste", this.handlePaste);
    this.setState({ isDragOver: false, uploadComplete: false });
  };
  ...
  handleDrop = (e) => {
    ...
    if (file && isImageFile(file)) this.uploadFile(file);
  };

  uploadFile = (file) => {
    const { uploadUrl, fileFieldName = "header_logo" } = this.props;
    if (!uploadUrl) return;

    this.setState({ uploadPending: true, uploadError: null });
  ...
  renderUploadModal() {
    const { uploadPending, uploadError, uploadComplete, isDragOver } = this.state;
    const {
      uploadImageTitle,
      close,
      uploadSuccess,
      uploading,
      uploadComplete: uploadCompleteLabel,
      selectImage,
      dragDropHint,
      pasteHint,
      dropZoneAriaLabel
    ...
    const selectButtonLabel =
      uploadPending ? uploading : uploadComplete ? uploadCompleteLabel : selectImage;
...
ImageUpload.propTypes = {
  ...
  uploadComplete: PropTypes.string,
