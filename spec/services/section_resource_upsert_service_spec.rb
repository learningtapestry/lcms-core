...
describe SectionResourceUpsertService do
  describe ".call" do
    ...
    it "truncates description_summary to 255 chars on the description column" do
      long_text = "x" * 500
      described_class.call(metadata: metadata.merge("description" => long_text), source_link_data:)
      expect(resource.description.length).to be <= 255
    end
  end
end
