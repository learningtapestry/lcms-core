...
describe Admin::DocumentsController do
  ...
  describe '#create' do
    ...
    let(:form) do
      instance_double('DocumentForm', document:, save: valid, service_errors: [])
    end
    let(:link) { 'link' }
    let(:params) { { link: } }
    let(:valid) { true }

    before { allow(DocumentForm).to receive(:new).and_return(form) }

    subject { post :create, params: { document_form: params } }

    it 'creates DocumentForm object' do
      expect(DocumentForm).to receive(:new).with(params)
      subject
    end
  ...
  describe '#destroy' do
    ...
    context 'when there was custom filter' do
      ...
      it { is_expected.to redirect_to "/admin/documents?#{{ query: }.to_param}" }
    end
  end
